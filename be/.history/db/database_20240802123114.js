...
db.run(`INSERT INTO category (name) VALUES ('기타')`, function (err) {
  if (err) {
    console.error('category 테이블 데이터 삽입 오류:', err.message);
  } else {
    console.log('category 테이블 데이터 삽입 성공');

    // 삽입된 데이터의 idx 값을 얻기 위한 쿼리
    db.get(`SELECT idx FROM category WHERE name = '기타'`, (err, row) => {
      if (err) {
        console.error('카테고리 idx 조회 오류:', err.message);
      } else {
        const categoryIdx = row.idx;

        // account_transaction 테이블에 데이터 삽입
        db.run(
          ...
          err => {
            if (err) {
              console.error(
                'account_transaction 테이블 데이터 삽입 오류:',
                err.message,
              );
            } else {
              console.log('account_transaction 테이블 데이터 삽입 성공');
            }
          },
        );
      }
